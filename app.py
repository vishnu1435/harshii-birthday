...
with st.sidebar:
    ...
    st.header("📊 Harshiii Analytics")

    st.metric(
        label="Days Since Owing Me Coffee",
        value="3"
    )
    ...
    routine_data = {
        "Activity": [
            "Working",
            "Staring at the walls in court",
            "Sleeping in court hall",
            "Melting over street puppies"
        ],
        "Percentage": [
            10,
            30,
    ...
    st.subheader("😈 Dove Raja Annoyance Meter")

    st.progress(0.95)

    st.caption("95% — Extremely dangerous levels detected.")

    st.divider()

    st.caption("Analytics generated by highly questionable junior intelligence.")
...
st.markdown(
    ...
    <div class="verdict">
        ...
        <p>
        The Court hereby orders the accused to enjoy her 21st birthday,
        smile excessively, eat something nice, and have an absolutely
        amazing year ahead.
        </p>
        <p>
...
if not st.session_state.verdict_accepted:
    ...
    if st.button(
        "🎂 ACCEPT VERDICT & UNLOCK TRUE BIRTHDAY WISH",
        use_container_width=True,
        type="primary"
    ):
...
if st.session_state.verdict_accepted:
    ...
    try:
        ...
        with col2:
            st.image(
                puppy,
                caption="Exhibit E — The only witnesses the Court trusts.",
             use_container_width=True
            )
        ...
        st.info(
            "🐶 Exhibit E (puppy_pic.jpg) is currently unavailable. "
            "The Court suspects the puppies are on recess."
        )
